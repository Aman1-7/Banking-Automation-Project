...
win.title("icici bank")

lbl_title=Label(win,text="Banking Automation",bg="powder blue",font=('Arial',50,'bold','underline'))
lbl_title.place(relx=.25,rely=.02)

img=Image.open("logo.jpg").resize((225,140))
...
def newuser_screen():
    ...
    def openacn_db():
        ...
        tup=cur.fetchone()
        con.close()
        messagebox.showinfo("Success",f"Account Opend with ACN :{tup[0]}")
        frm.destroy()
        login_screen()
    
    btn_back=Button(frm,text="<",command=back,font=('Arial',20,'bold'),bd=5,bg="powder blue")
    ...
    lbl_mob=Label(frm,text="Mob No",bg='pink',font=('Arial',20,'bold'))
...
def welcome_screen():
    ...
    def deposit():
        ...
        def deposit_db():
            ...
            if(amt<0):
                ...
                messagebox.showerror("Deposit","Amount deposited")
        ...
        btn_dep=Button(frm,text="deposit",command=deposit_db,font=('Arial',20,'bold'),bd=5,bg="powder blue")
    ...
    def withdraw():
        ...
        lbl_page.configure(text="This is Whithdraw page")
        ...
        def withdraw_db():
            ...
            if(amt<0):
                ...
                if(bal>=amt):
                    ...
                    messagebox.showerror("Withdraw","Amount withdrawn")
                else:
    ...
    def transfer():
        ...
        def transfer_db():
            ...
            if(row==None):
                ...
                con=sqlite3.connect(database="bank.sqlite")
                cur=con.cursor()
                cur.execute("slect account_bal from accounts where account_no=?",(t_acn,))
                t_bal=cur.fetchone()
                con.close()
                if(bal>=amt):
                    con=sqlite3.connect(database="bank.sqlite")
                    cur=con.cursor()
                    dt=str(datetime.now())
                    cur.execute("update account set account_bal=account_bal-? where account_no=?",(amt,tup[0]))
